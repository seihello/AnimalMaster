...
class GameEnd:
    ...
    def check_game_end(self, lower_units, upper_units, turn):
        ...
        # どちらかが敵陣マスに入ったらゲーム終了
        result = self._check_goal_arrival(lower_units, upper_units)
        if result != GameResult.NOT_COMPLETE:
            return result

        # 駒が全て死んだらゲーム終了
        result = self._check_unit_alive(lower_units, upper_units)
        if result != GameResult.NOT_COMPLETE:
            return result

        # 設定ターン数終わってゲームが終了しなかったらゲーム終了
        result = self._check_max_turn(lower_units, upper_units, turn)
        if result != GameResult.NOT_COMPLETE:
            return result
    ...
    # 敵陣営の進入によるゲーム終了を判定する
    def _check_goal_arrival(self, lower_units, upper_units):
    ...
    def _check_unit_alive(self, lower_units, upper_units):
        lower_units_num = self._get_living_units_count(lower_units)
        upper_units_num = self._get_living_units_count(upper_units)
    ...
    def _check_max_turn(self, lower_units, upper_units, turn):
    ...
    def _get_living_units_count(self, units):
        living_count = 0
        for unit in units:
            if unit.is_living:
                living_count += 1
